...
export const Map = () => {
  ...
  const onEachFeature = (feature, layer) => {
    if (feature.properties) {
      // Initialize a string for the HTML content of the popup
      let popupContent = '<table style="border-collapse: collapse; width: 100%;">';
  
      // Loop through each property in the feature's properties object
      for (const key in feature.properties) {
        if (feature.properties.hasOwnProperty(key)) {
          const value = feature.properties[key];
          // Add a row to the table for each property
          popupContent += `
            <tr>
      ...
      layer.bindPopup('<p>No properties available</p>');
    }
  };
  ...
  return (
    <div className="container">
      ...
      >
        ...
        {/* Render each uploaded file as a GeoJSON layer */}
        {uploadedFiles.map((file, index) => (
          <GeoJSON
            key={index}
            data={file.data}
            style={getFeatureStyle} // Apply style for each file
            onEachFeature={onEachFeature} // Apply onEachFeature function for each file
          />
        ))}
